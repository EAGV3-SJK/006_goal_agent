...
fetch_url:   httpx first (fast, no browser), crawl4ai fallback for JS-rendered pages.
...
# Patch ssl module to use the OS certificate store (Windows system CA certs).
# Required on this machine because the network uses a corporate TLS inspection
# proxy whose CA is in the Windows cert store but not in certifi's bundle.
...
def _html_to_text(html: str) -> str:
    """Strip HTML tags, preserve block-level line breaks, collapse whitespace."""
...
async def _httpx_fetch(url: str, timeout: int) -> dict:
    """Fast static-page fetch via httpx — no browser, no Playwright.
    Handles most documentation sites, Wikipedia, PyPI, GitHub, forums."""
    headers = {
        "User-Agent": (
...
async def fetch_url(url: str, timeout: int = 20) -> dict:
    """Fetch text content from a URL. Uses httpx for static pages (fast, no browser),
    falls back to crawl4ai/headless-Chromium only if httpx fails (JS-rendered pages).
    Example: fetch_url("https://discuss.python.org/t/asyncio-best-practices/12576")."""
    try:
        return await _httpx_fetch(url, timeout)
    except Exception as e:
        # Fallback to crawl4ai for JS-rendered pages. If that also fails,
        # raise the original httpx error so the agent sees something actionable.
        try:
