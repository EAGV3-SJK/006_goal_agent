...
class OpenAICompatProvider(BaseProvider):
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        print(f"[{self.name}] Connecting to: {self.base_url}/chat/completions (model={body.get('model')})")
        print(f"[{self.name}] Headers: Authorization: Bearer {self.api_key[:10]}... (length={len(self.api_key)})")
        try:
            ...
            print(f"[{self.name}] Exception during post: {type(e).__name__} - {e}")
            raise
...
class LMStudioProvider(OpenAICompatProvider):
    ...
    def __init__(self, model, base_url="http://127.0.0.1:1234/v1"):
        super().__init__("lm-studio", model, base_url)
...
class AnthropicProvider(BaseProvider):
    ...
    def _translate_messages(self, messages):
        ...
        for m in messages:
            ...
            if r == "tool":
                ...
                if out and out[-1]["role"] == "user" and isinstance(out[-1]["content"], list) and any(isinstance(x, dict) and x.get("type") == "tool_result" for x in out[-1]["content"]):
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        for b in blocks:
            ...
            if b["cache"] or (cache_system and b == blocks[-1]):
        ...
        async with httpx.AsyncClient(timeout=180, trust_env=False) as c:
            r = await c.post(f"{self.base_url}/v1/messages", headers=self._headers(), json=body)
            if r.status_code != 200:
                raise ProviderError(
...
def build_providers(cache_store):
    ...
    if k := os.getenv("NVIDIA_API_KEY"):
        out["nvidia"] = NvidiaProvider(k, os.getenv("NVIDIA_MODEL", "meta/llama-3.3-70b-instruct"))
    ...
    if k := os.getenv("ANTHROPIC_API_KEY"):
        out["anthropic"] = AnthropicProvider(k, os.getenv("ANTHROPIC_MODEL", "claude-sonnet-4-6"))
    if url := os.getenv("LMSTUDIO_URL"):
        out["lmstudio"] = LMStudioProvider(os.getenv("LMSTUDIO_MODEL", "google/gemma-4-e4b"), url)
    return out
