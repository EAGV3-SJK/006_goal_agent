...
# Free-first order: local (lmstudio, ollama) → free-tier cloud (cerebras, groq,
# nvidia, openrouter, github) → paid (anthropic, gemini).
DEFAULT_ORDER = ["lmstudio", "ollama", "cerebras", "groq", "nvidia", "openrouter", "github", "anthropic", "gemini"]
...
# TINY: prefers fast/cheap local and free-tier cloud providers.
# LARGE: starts with Gemini (1M ctx) and Anthropic (200K ctx) — they are the
#        only providers that reliably handle 30K–250K token payloads from
#        artifact-attach. Free-tier providers (cerebras 8K, lmstudio 8K) fail
#        on large contexts so they come last.
# HUGE: rejected — Summarizer Agent will live in V7.
TIER_TO_ORDER = {
    "TINY":  ["lmstudio", "ollama", "cerebras", "groq", "nvidia", "github", "openrouter", "anthropic", "gemini"],
    "LARGE": ["gemini", "anthropic", "nvidia", "groq", "openrouter", "github", "ollama", "lmstudio", "cerebras"],
}
